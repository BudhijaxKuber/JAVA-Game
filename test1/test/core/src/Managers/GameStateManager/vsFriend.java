...
public class vsFriend extends gameStates {
    ...
    public void init() {
        ...
        player = new player("Player " + playerManager.getPlayerNo());
        ...
        if (playerManager.getPlayerNo()==1) options = new String[] {"<","TANKS","CHOOSE","UPGRADE"};
    ...
    public void update(float dt) {
        ...
        if (currentOption==1) {
            currentOption += tankManager.update(dt, bgColor);
            return;
        }
    ...
    private void select() {
        if (currentOption==0) {
            gsm.setGameState(gsm.MENU);
        }
        else if (currentOption==2) {
            tankManager.allotTank(player);
            playerManager.setPlayer(player);
            if (playerManager.getPlayerNo()==2) init();
    ...
    public void draw() {
        ...
        // DRAW TANKS
        tankManager.draw(currentOption==1);
